...
const yorumEkle = function(req, res){
    ...
    if(mekanid){
        Mekan.findById(mekanid).select('yorumlar').exec(
...
if(req.params&&req.params.mekanid&&req.params.yorumid)
{
    ...
    .select('ad yorumlar')  //hız ve bant genişliği için dökümandan sadece adı ve yorumları sorgulayan kod
    .exec(
        function(hata, mekan){
            var cevap, yorum;
            if(!mekan){
            ...
            if(mekan.yorumlar && mekan.yorumlar.length >0){ //dökümanda yorumlar var mı ve varsa içi dolu mu? 
                yorum = mekan.yorumlar.id(req.params.yorumid);  //yorumların içinde aradığımız id'de yorum var mı?
                if(!yorum){
...
const yorumGuncelle = function(req, res){
    if(!req.params.mekanid || !req.params.yorumid){
        cevapOlustur(res, 404, {"mesaj" : "mekanid ve yorumid zorunlu"});
        return;
    }
    Mekan
    ...
    .exec(function(hata, gelenMekan){
        ...
        if(gelenMekan.yorumlar && gelenMekan.yorumlar.length >0){
            ...
            if(!yorum){
                cevapOlustur(res, 404, {"mesaj" : "yorumid bulnuamadı"});
            }
            else{
...
const yorumSil = function(req, res){
    ...
    Mekan.findById(req.params.mekanid)
    .select('yorumlar').exec(function(hata, gelenMekan){
        if(!gelenMekan){
            cevapOlustur(res, 404, {"mesaj" : "mekanid bulunamadı"});
            return;
        }
