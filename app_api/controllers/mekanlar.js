...
const mekanlariListele = async (req, res) => {
  var boylam = parseFloat(req.query.boylam); // ? işaretinden sonra gelenler sorgu parametreleridir
  var enlem = parseFloat(req.query.enlem); //url tanımlarımızda enlem ve boylam olmadığı için bu yolla enlem ve boylamı alıyoruz
  var nokta = {
      type: "Point",
...
const mekanGetir = function(req, res){
 if(req.params && req.params.mekanid){  //url'de parametre var mı ve varsa bu mekanid' mi ?
...
const mekanGuncelle = function(req, res){
   if(!req.params.mekanid){
    ...
    Mekan.findById(req.params.mekanid)
    .select('-yorumlar -puan') //yorumlar ve puan dışında her şeyi almamızı söyleyen kod
